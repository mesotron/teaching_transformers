...
If you just want the training, validation, and test files from the paper 
without running this code, these can be found in data/formatted/.

Note that correct demonstrations are included after the prefixes in the test 
files only for reference; these are *not* to be used by the model at test 
time. See repo README for more information.

If you want to run this code to regenerate the files in data/formatted/ for
yourself, you can use the files present in data/originals as input. Note 
that the content of data/originals is not the exact output of 
format_benchmarks.py: its output files were separately renamed and truncated 
to yield the files in data/originals.
"""
